...
v0.4 抽卡與圖鑑規格請見 [docs/gacha_and_gallery_spec.md](docs/gacha_and_gallery_spec.md)。

## 目前進度：v0.4 Demo（抽卡 × 收藏 × 結局圖鑑版）
...
/Guma_Game
  /docs
    GDD.md                    # 完整遊戲企劃書
    gacha_and_gallery_spec.md # v0.4 抽卡與圖鑑規格
  /data                       # 遊戲資料（改內容不用動程式碼）
  ...
  /prototype                  # 遊戲原型（純 HTML/CSS/JS，無需安裝套件）
    index.html                # 舞台結構（首頁/抽卡/收藏/圖鑑/背景/立繪/對話框/手牌等圖層）
    style.css                 # 畫面樣式與演出動畫
    main.js                   # 引擎：抽卡、收藏、結局圖鑑、牌庫手牌、場景跳轉、補救、劇本播放
```
...
- **調整抽卡與圖鑑規格**：見 [docs/gacha_and_gallery_spec.md](docs/gacha_and_gallery_spec.md)
