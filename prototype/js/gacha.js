...
export function pickRarity(forceMinRarity = null) {
  const rates = forceMinRarity
    ? GACHA_RATES.filter((item) => RARITY_ORDER[item.rarity] >= RARITY_ORDER[forceMinRarity])
    : GACHA_RATES;
...
export function drawGacha(count) {
  if (store.getTickets() < count) {
    renderGachaMessage(`抽卡券不足：需要 ${count} 張，目前只有 ${store.getTickets()} 張。請透過遊玩與達成結局慢慢累積。`, true);
    return [];
  }
  ...
  for (let index = 0; index < count; index += 1) {
    results.push(drawOneGachaCard(firstTen && index === count - 1 ? "SR" : null));
  }
...
function endingHint(ending) {
  if (ending.category === "emergency") return "提示：女主忍受條歸零後，緊急救援的骰子會決定命運。";
  const conditions = ending.conditions || {};
  if (Object.keys(conditions.cards || {}).length) return "提示：和特定卡片使用次數有關。";
...
export function renderEndingGallery() {
  ...
  DATA.endings.forEach((ending, index) => {
    ...
    const element = document.createElement("div");
    element.className = `ending-card ${isUnlocked ? "unlocked" : "locked"}`;
    element.innerHTML = isUnlocked
...
export function unlockEnding(endingId) {
  ...
  if (wasNew) {
    ...
    store.setUnlockedEndingIds(unlocked);
  }
  return wasNew;
}
